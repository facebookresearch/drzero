...
def get_custom_reward_fn(config, tokenizer):
    ...
    reward_kwargs['config'] = config.actor_rollout_ref.rollout
    reward_kwargs['processing_class'] = tokenizer

    return partial(_call_with_kwargs, raw_fn, reward_kwargs)


def load_reward_manager(config, tokenizer, num_examine, is_validation=False, **reward_kwargs):
    """
    ...
    # for validation, fall back to rule-based
    val_with_default = config.reward_model.get("validation_with_default_compute_score", False) 

    if is_validation and val_with_default:
        print("Using naive reward manager & default_compute_score for validation")
    ...
    if is_validation and val_with_default:
        ...
        compute_score = get_custom_reward_fn(config, tokenizer)
